...
IMAGE img_stone_0, img_stone_1, img_stone_2, img_stone_3, img_stone_4, img_stone_5, img_stone_6, img_stone_7,img_stone_99;//定义石块的图片
...
void startup() {
	...
	loadimage(&img_stone_7, _T("..//images//stone_99.jpg"));//导入破碎99的图片
...
void show() {
	...
	if (scene == 1) {
		...
		putimage(735, 213, oxygen_i,oxygen_j, &img_oxygen, 0, 0);
	}
	//角色五元组
...
void status_change() {
	...
	{
		putimage(bomb.center_x - 32, bomb.center_y - 32, bomb_width, bomb_high, &img_bwbomb_boom, bomb_i * bomb_width, 0, NOTSRCERASE);//放置向左走的黑白图片
		putimage(bomb.center_x - 32, bomb.center_y - 32, bomb_width, bomb_high, &img_bomb_boom, bomb_i * bomb_width, 0, SRCINVERT);//放置向左走的彩图
		bomb_i++;
		Sleep(100);
		if (bomb_i == 8)
		{
